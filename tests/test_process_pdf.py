...
def test_process_single_pdf_success(
    ...
    document = process_single_pdf(
        pdf_path, output_dir, dpi, mock_session, logger, order_strategy=get_default_order
    )
...
def test_process_single_pdf_with_reading_order(
    ...
    document = process_single_pdf(
        pdf_path, output_dir, dpi, mock_session, logger, order_strategy=order_strategy
    )
...
def test_process_single_pdf_invalid_fragments(mock_get_document, mock_session):
    ...
    with patch("src.workflows.process_pdf.analyze_pdf") as mock_analyze_pdf:
        ...
        document = process_single_pdf(
            pdf_path, Path("data/images"), 150, mock_session, logger, get_default_order
        )

        assert document is not None
        logger.debug.assert_called_with(
            {"file": str(pdf_path), "fragments_received": 1, "fragments_valid": 0}
        )
